(function() {
    ...
    toolbar.innerHTML = '<div style="display:flex;flex-direction:column;"><span style="font-weight:bold;color:#fff;font-size:14px;text-shadow:0 0 5px ' + greenGlow + ';">ROOT@TERMINAL</span><span style="font-size:8px;color:' + greenGlow + ';opacity:0.7;">MODE: ALL_IN_ONE | ADS: READY</span></div>';
    ...
    // --- Bypass Ads Function (เพิ่มใหม่ตามคำขอ) ---
    function toggleAdsBypass() {
        if (adRemoverInterval) {
            clearInterval(adRemoverInterval);
            adRemoverInterval = null;
            console.log("Bypass Ads Stopped");
        } else {
            console.log("Bypass Ads Started");
            adRemoverInterval = setInterval(function() {
                var v = document.querySelector('video');
                if (v) {
                    var skipButtons = document.querySelectorAll('[class*="skip"], [id*="skip"], .ytp-ad-skip-button, .atp-ad-skip-button');
                    skipButtons.forEach(btn => btn.click());
                    if (!v.paused && v.duration < 300) { 
    ...
    // --- Network Interceptor (จากโค้ดเดิมของคุณ) ---
    ...
    // --- Initialize ---
    controls.appendChild(createBtn('ADS', toggleAdsBypass)); // ปุ่ม ADS เพิ่มเข้ามาแล้ว
    ...
    // --- New Feature: ADS Bypass Logic ---
    function toggleAdsBypass() {
        if (adRemoverInterval) {
            clearInterval(adRemoverInterval);
            adRemoverInterval = null;
            alert("ADS BYPASS: OFF");
        } else {
            alert("ADS BYPASS: ON");
            adRemoverInterval = setInterval(function() {
                var v = document.querySelector('video');
    ...
    var oXOpen = XMLHttpRequest.prototype.open;
    XMLHttpRequest.prototype.open = function(m, u) {
        if (typeof u === 'string' && (u.includes('.m3u8') || u.includes('.mp4'))) logUrl(u, 'XHR');
        return oXOpen.apply(this, arguments);
    };
})();
